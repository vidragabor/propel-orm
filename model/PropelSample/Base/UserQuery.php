...
use \Exception;
use \PDO;
use PropelSample\User as ChildUser;
use PropelSample\UserQuery as ChildUserQuery;
use PropelSample\Map\UserTableMap;
 ...
 * @method     ChildUserQuery orderByLoginCounter($order = Criteria::ASC) Order by the login_counter column
 ...
 * @method     ChildUserQuery groupByLoginCounter() Group by the login_counter column
 ...
 *
 * @method     \PropelSample\UserGroupQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
 ...
 * @method     ChildUser findOneByLoginCounter(int $login_counter) Return the first ChildUser filtered by the login_counter column
 * @method     ChildUser findOneByBlocked(boolean $blocked) Return the first ChildUser filtered by the blocked column *
 ...
 * @method     ChildUser requireOneByLoginCounter(int $login_counter) Return the first ChildUser filtered by the login_counter column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildUser[]|ObjectCollection findByLoginCounter(int $login_counter) Return ChildUser objects filtered by the login_counter column
 * @method     ChildUser[]|ObjectCollection findByBlocked(boolean $blocked) Return ChildUser objects filtered by the blocked column
...
abstract class UserQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \PropelSample\Base\UserQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\PropelSample\\User', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT `id`, `email`, `password`, `first_name`, `last_name`, `login_counter`, `blocked` FROM `user` WHERE `id` = :p0';
        try {
    ...
    /**
     * Filter the query on the login_counter column
     *
     * Example usage:
     * <code>
     * $query->filterByLoginCounter(1234); // WHERE login_counter = 1234
     * $query->filterByLoginCounter(array(12, 34)); // WHERE login_counter IN (12, 34)
     * $query->filterByLoginCounter(array('min' => 12)); // WHERE login_counter > 12
     * </code>
     *
     * @param     mixed $loginCounter The value to use as filter.
     *              Use scalar values for equality.
     *              Use array values for in_array() equivalent.
     *              Use associative array('min' => $minValue, 'max' => $maxValue) for intervals.
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     ...
     * @return $this|ChildUserQuery The current query, for fluid interface
     */
    public function filterByLoginCounter($loginCounter = null, $comparison = null)
    {
        if (is_array($loginCounter)) {
            $useMinMax = false;
            if (isset($loginCounter['min'])) {
                $this->addUsingAlias(UserTableMap::COL_LOGIN_COUNTER, $loginCounter['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($loginCounter['max'])) {
                $this->addUsingAlias(UserTableMap::COL_LOGIN_COUNTER, $loginCounter['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(UserTableMap::COL_LOGIN_COUNTER, $loginCounter, $comparison);
    }
    ...
    public function filterByBlocked($blocked = null, $comparison = null)
    {
        ...
        return $this->addUsingAlias(UserTableMap::COL_BLOCKED, $blocked, $comparison);
    }

    /**
     * Filter the query by a related \PropelSample\UserGroup object
     *
     * @param \PropelSample\UserGroup|ObjectCollection $userGroup the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
    ...
    {
        if ($userGroup instanceof \PropelSample\UserGroup) {
            return $this
            ...
            throw new PropelException('filterByUserGroup() only accepts arguments of type \PropelSample\UserGroup or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \PropelSample\UserGroupQuery A secondary query class using the current class as primary query
     */
    public function useUserGroupQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinUserGroup($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'UserGroup', '\PropelSample\UserGroupQuery');
    }
