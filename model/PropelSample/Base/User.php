...
use PropelSample\UserGroup as ChildUserGroup;
use PropelSample\UserGroupQuery as ChildUserGroupQuery;
use PropelSample\UserQuery as ChildUserQuery;
use PropelSample\Map\UserGroupTableMap;
...
abstract class User implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\PropelSample\\Map\\UserTableMap';
    ...
    /**
     * The value for the login_counter field.
     *
     * Note: this column has a database default value of: 0
     * @var        int
     */
    protected $login_counter;
    ...
    /**
     * Initializes internal state of PropelSample\Base\User object.
     * @see applyDefaults()
     */
    ...
    /**
     * Get the [login_counter] column value.
     *
     * @return int
     */
     ...
     * @return $this|\PropelSample\User The current object (for fluent API support)
     */
    ...
    /**
     * Set the value of [login_counter] column.
     *
     * @param int $v new value
     * @return $this|\PropelSample\User The current object (for fluent API support)
     */
    public function setLoginCounter($v)
    {
        if ($v !== null) {
            $v = (int) $v;
        }

        if ($this->login_counter !== $v) {
            $this->login_counter = $v;
            $this->modifiedColumns[UserTableMap::COL_LOGIN_COUNTER] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : UserTableMap::translateFieldName('LoginCounter', TableMap::TYPE_PHPNAME, $indexType)];
            $this->login_counter = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : UserTableMap::translateFieldName('Blocked', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\PropelSample\\User'), 0, $e);
        }
    }
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->userGroupsScheduledForDeletion !== null) {
                if (!$this->userGroupsScheduledForDeletion->isEmpty()) {
                    \PropelSample\UserGroupQuery::create()
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(UserTableMap::COL_LOGIN_COUNTER)) {
            $modifiedColumns[':p' . $index++]  = '`login_counter`';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case '`last_name`':
                        ...
                        break;
                    case '`login_counter`':
                        $stmt->bindValue($identifier, $this->login_counter, PDO::PARAM_INT);
                        break;
                    case '`blocked`':
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        $result = array(
            ...
            $keys[5] => $this->getLoginCounter(),
            $keys[6] => $this->getBlocked(),
        );
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[5], $arr)) {
            $this->setLoginCounter($arr[$keys[5]]);
        }
        if (array_key_exists($keys[6], $arr)) {
            $this->setBlocked($arr[$keys[6]]);
        }
    }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\PropelSample\User The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(UserTableMap::COL_LOGIN_COUNTER)) {
            $criteria->add(UserTableMap::COL_LOGIN_COUNTER, $this->login_counter);
        }
        if ($this->isColumnModified(UserTableMap::COL_BLOCKED)) {
     ...
     * objects.
     *
     * @param      object $copyObj An object of \PropelSample\User (or compatible) type.
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        $copyObj->setLoginCounter($this->getLoginCounter());
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \PropelSample\User Clone of current object.
     * @throws PropelException
     */
    ...
    public function initUserGroups($overrideExisting = true)
    {
        ...
        $this->collUserGroups->setModel('\PropelSample\UserGroup');
    }
