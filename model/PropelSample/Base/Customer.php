...
abstract class Customer implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\PropelSample\\Map\\CustomerTableMap';
    ...
    /**
     * Initializes internal state of PropelSample\Base\Customer object.
     * @see applyDefaults()
     */
     ...
     * @return $this|\PropelSample\Customer The current object (for fluent API support)
     */
    ...
    public function setTypeId($v)
    {
        ...
        if ($this->type_id !== $v) {
            $this->type_id = $v;
            $this->modifiedColumns[CustomerTableMap::COL_TYPE_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : CustomerTableMap::translateFieldName('TypeId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->type_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : CustomerTableMap::translateFieldName('Active', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\PropelSample\\Customer'), 0, $e);
        }
    }
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->contractsScheduledForDeletion !== null) {
                if (!$this->contractsScheduledForDeletion->isEmpty()) {
                    \PropelSample\ContractQuery::create()
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(CustomerTableMap::COL_TYPE_ID)) {
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\PropelSample\Customer
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\PropelSample\Customer The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(CustomerTableMap::COL_TYPE_ID)) {
            $criteria->add(CustomerTableMap::COL_TYPE_ID, $this->type_id);
        }
        if ($this->isColumnModified(CustomerTableMap::COL_ACTIVE)) {
     ...
     * objects.
     *
     * @param      object $copyObj An object of \PropelSample\Customer (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \PropelSample\Customer Clone of current object.
     * @throws PropelException
     */
    ...
    public function initContracts($overrideExisting = true)
    {
        ...
        $collectionClassName = ContractTableMap::getTableMap()->getCollectionClassName();

        $this->collContracts = new $collectionClassName;
        $this->collContracts->setModel('\PropelSample\Contract');
    }
    ...
    public function setContracts(Collection $contracts, ConnectionInterface $con = null)
    {
        ...
        foreach ($contractsToDelete as $contractRemoved) {
            $contractRemoved->setCustomer(null);
        }
    ...
    {
        if (null !== $this->aType) {
            $this->aType->removeCustomer($this);
        }
        $this->id = null;
        $this->name = null;
        $this->address = null;
