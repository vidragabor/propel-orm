 ...
 *
 * @method     \PropelSample\UserQuery|\PropelSample\GroupQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
...
abstract class UserGroupQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \PropelSample\Base\UserGroupQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\PropelSample\\UserGroup', $modelAlias = null)
    {
    ...
    public function findPk($key, ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = UserGroupTableMap::getInstanceFromPool(null === $key || is_scalar($key) || is_callable([$key, '__toString']) ? (string) $key : $key)))) {
    ...
    {
        $sql = 'SELECT `id`, `user_id`, `group_id` FROM `user_group` WHERE `id` = :p0';
        try {
    ...
    /**
     * Filter the query by a related \PropelSample\User object
     *
     * @param \PropelSample\User|ObjectCollection $user The related object(s) to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @throws \Propel\Runtime\Exception\PropelException
     *
    ...
    {
        if ($user instanceof \PropelSample\User) {
            return $this
            ...
            throw new PropelException('filterByUser() only accepts arguments of type \PropelSample\User or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \PropelSample\UserQuery A secondary query class using the current class as primary query
     */
    public function useUserQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinUser($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'User', '\PropelSample\UserQuery');
    }

    /**
     * Filter the query by a related \PropelSample\Group object
     *
     * @param \PropelSample\Group|ObjectCollection $group The related object(s) to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @throws \Propel\Runtime\Exception\PropelException
     *
    ...
    {
        if ($group instanceof \PropelSample\Group) {
            return $this
            ...
            throw new PropelException('filterByGroup() only accepts arguments of type \PropelSample\Group or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \PropelSample\GroupQuery A secondary query class using the current class as primary query
     */
    public function useGroupQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinGroup($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Group', '\PropelSample\GroupQuery');
    }
